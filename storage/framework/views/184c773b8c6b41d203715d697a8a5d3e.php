...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', config('app.name', 'TotoNest')); ?></title>
...
<body>
    ...
    <?php if(session('success')): ?>
        ...
        <script>
            Swal.fire({
                icon: 'success',
                title: 'Success',
                text: <?php echo json_encode($successMessage, 15, 512) ?>,
                color: '#4b5563',
                background: '#ffffff',
    ...
    <?php if(session('error')): ?>
        ...
        <script>
            Swal.fire({
                icon: 'error',
                title: <?php echo json_encode($isCartEmptyError ? 'Your Cart Is Empty' : 'Something Went Wrong', 15, 512) ?>,
                text: <?php echo json_encode($isCartEmptyError ? 'Your cart is empty. Start shopping to add items you love.' : $errorMessage, 15, 512) ?>,
                color: '#4b5563',
                background: '#ffffff',
                confirmButtonColor: getComputedStyle(document.documentElement).getPropertyValue('--teal-primary').trim() || '#0d9488',
...
<?php /**PATH C:\Users\hngob\eccomerce_edu\resources\views/layouts/shop.blade.php ENDPATH**/ ?>