...
@section('styles')
    <link rel="stylesheet" href="{{ asset('css/customer-dashboard-clean.css') }}?v={{ filemtime(public_path('css/customer-dashboard-clean.css')) }}">
@endsection
    ...
    <div class="container-fluid mt-2 customer-dashboard-clean">
        <div class="d-flex flex-column flex-md-row justify-content-start align-items-start align-items-md-center gap-2 gap-md-3 mb-4 dashboard-heading">
            ...
            <div class="dashboard-cta">
                <a href="{{ route('shop') }}" class="btn dashboard-primary-btn w-100">
        ...
        <div class="row g-3 mb-4 dashboard-metrics-row">
            <div class="col-12 col-md-4">
                <a href="{{ route('customer.orders') }}" class="summary-card summary-link" aria-label="View total orders">
                    <i class="bi bi-receipt summary-watermark"></i>
                    <div class="summary-label">Total Orders</div>
                    <div class="summary-value"
                         data-counter-target="{{ (int) ($totalOrders ?? 0) }}"
                         data-counter-type="number">{{ number_format($totalOrders ?? 0) }}</div>
                </a>
            </div>
            <div class="col-12 col-md-4">
                <a href="{{ route('customer.orders', ['status' => 'completed']) }}" class="summary-card summary-link"
                    aria-label="View completed orders">
                <i class="bi bi-cash-stack summary-watermark"></i>
                <div class="summary-label">Total Spent</div>
                <div class="summary-value"
                     data-counter-target="{{ (float) ($totalSpent ?? 0) }}"
                     data-counter-type="money"
                     data-counter-currency="Tsh"
                     data-counter-precision="1">{{ format_money_short($totalSpent ?? 0) }}</div>
            </a>
        </div>
            <div class="col-12 col-md-4">
                <a href="{{ route('cart.index') }}" class="summary-card summary-link" aria-label="View cart items">
                    <i class="bi bi-cart-fill summary-watermark"></i>
                    <div class="summary-label">Cart Items</div>
                    <div class="summary-value"
                         data-counter-target="{{ (int) ($cartItemsCount ?? 0) }}"
                         data-counter-type="number">{{ number_format($cartItemsCount ?? 0) }}</div>
                </a>
            </div>
        </div>
            ...
            <div class="col-lg-8">
                <div class="card clean-card h-100">
                    <div class="card-header clean-card-header d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('customer.orders') }}" class="btn btn-sm dashboard-primary-btn">
                    ...
                    <div class="card-body p-0">
                                ...
                                <table class="table table-hover mb-0">
                                    ...
                                    <tbody>
                                        @foreach ($recentOrders as $order)
                                            ...
                                            @endphp
                                            <tr>
                                                <td class="ps-3 font-monospace">{{ $order->order_number ?: $order->public_id }}</td>

                                                <td>{{ $order->orderItems->count() }}</td>
                                                <td>{{ format_money_short($order->total_amount, 0) }}</td>
                                                <td>
                                                    <span
                                                        class="badge text-capitalize {{ $statusClass }}">{{ $order->status }}</span>
                                                </td>
                        ...
                        @else
                            <div class="dashboard-empty-state p-4">
                                ...
                                <a href="{{ route('shop') }}" class="btn btn-sm dashboard-primary-btn">
                        ...
                        @if (($totalOrders ?? 0) > 0)
                            <div class="order-donut-wrap">
                                <div class="order-donut-chart"
                                    data-completed-deg="{{ $completedDeg }}"
                                    data-pending-deg="{{ $pendingDeg }}"
                                    data-cancelled-deg="{{ $cancelledDeg }}"
                                    style="--completed-deg: 0deg; --pending-deg: 0deg; --cancelled-deg: 0deg;">
                                    <div class="order-donut-center">
                                        <div class="order-donut-total">{{ number_format($totalOrders ?? 0) }}</div>
                            ...
                            <div class="text-center mt-3 text-muted small">
                                <span class="status-dot status-dot-completed"></span>Completed: {{ number_format($completedOrders) }} |
                                <span class="status-dot status-dot-pending"></span>Pending: {{ number_format($pendingOrders) }} |
                                <span class="status-dot status-dot-cancelled"></span>Cancelled: {{ number_format($cancelledOrders) }}
                            </div>
                        @else
            ...
            <div class="col-12 d-none d-lg-block">
                <div class="card clean-card h-100">
                    <div class="card-header clean-card-header d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('customer.profile') }}" class="btn btn-sm dashboard-primary-btn">
                        ...
                        <div class="account-panels">
                            <div class="account-panel">
                                <div class="account-info-line">
                                    ...
                                    <span class="account-info-value">{{ strtoupper((string) ($profileUser->name ?? 'N/A')) }}</span>
                                </div>
                                <div class="account-info-line">
                                    <span class="account-info-key">Email:</span>
                                    <span class="account-info-value">{{ $profileUser->email ?? 'N/A' }}</span>
                                </div>
                                <div class="account-info-line">
                                    <span class="account-info-key">Phone:</span>
                                    <span class="account-info-value">{{ $profileUser->phone_number ?? 'N/A' }}</span>
                                </div>
                                <div class="account-info-line">
                                    <span class="account-info-key">Role:</span>
                                    <span class="account-info-value">{{ isset($profileUser->role) ? ucfirst((string) $profileUser->role) : 'N/A' }}</span>
                                </div>
                                <div class="account-info-line">
                                    <span class="account-info-key">Joined:</span>
                                    <span class="account-info-value">{{ optional($profileUser->created_at)->format('d M Y') ?? 'N/A' }}</span>
                                </div>
                            </div>
                            <div class="account-panel account-stats-panel">
                                <div class="account-info-line">
                                    <span class="account-info-key">Total Orders:</span>
                                    <span class="account-info-value">{{ number_format($totalOrders ?? 0) }}</span>
                                </div>
                                <div class="account-info-line">
                                    <span class="account-info-key">Total Cancelled:</span>
                                    <span class="account-info-value">{{ number_format($cancelledOrders ?? 0) }}</span>
                                </div>
                                <div class="account-info-line">
                                    <span class="account-info-key">Total Completed:</span>
                                    <span class="account-info-value">{{ number_format($completedOrders ?? 0) }}</span>
                                </div>
                                <div class="account-info-line">
                                    <span class="account-info-key">Total Spent:</span>
                                    <span class="account-info-value">{{ format_money_short($totalSpent ?? 0, 0) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        @if ($monthlySpending->count() > 0)
                            <div class="mb-3 spending-chart-wrap">
                                <canvas id="customerSpendingChart"
                                    data-monthly-spending='@json($monthlySpendingChart)'
                                    data-y-axis='@json($yAxisConfig)'
...
@section('scripts')
    ...
    <script src="{{ asset('js/customer-dashboard.js') }}?v={{ filemtime(public_path('js/customer-dashboard.js')) }}"></script>
@endsection
