        ...
        <div class="row g-3 mb-3">
            <div class="col-md-6 col-xl-3">
                <div class="panel-card p-3 h-100">
                    ...
                    <div class="ops-kpi-value">{{ number_format($orders24h) }}</div>
                    <div class="ops-kpi-sub">Total orders: {{ number_format($totalOrders) }}</div>
                </div>
            </div>
            <div class="col-md-6 col-xl-3">
                <div class="panel-card p-3 h-100">
                    <div class="ops-kpi-label">New Users (24h)</div>
                    <div class="ops-kpi-value">{{ number_format($users24h) }}</div>
                    <div class="ops-kpi-sub">Total users: {{ number_format($totalUsers) }}</div>
                </div>
            </div>
            <div class="col-md-6 col-xl-3">
                <div class="panel-card p-3 h-100">
                    <div class="ops-kpi-label">Catalog Updates (24h)</div>
                    <div class="ops-kpi-value">{{ number_format($products24h) }}</div>
                    <div class="ops-kpi-sub">Total products: {{ number_format($totalProducts) }}</div>
                </div>
            </div>
            <div class="col-md-6 col-xl-3">
                <div class="panel-card p-3 h-100">
                    <div class="ops-kpi-label">Config Records</div>
                    <div class="ops-kpi-value">{{ number_format($totalSettings) }}</div>
                    ...
                    <div class="panel-card-body d-grid gap-2">
                        <div class="ops-status d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <div class="ops-status-value">{{ $uptimePct }}%</div>
                            </div>
                            <span
                                class="ops-badge {{ $uptimePct >= 99.5 ? 'ops-ok' : 'ops-warn' }}">{{ $uptimePct >= 99.5 ? 'Stable' : 'Watch' }}</span>
                        </div>
                        <div class="ops-status d-flex justify-content-between align-items-center">
                            <div>
                                <div class="ops-status-head">Health Score</div>
                                <div class="ops-status-value">{{ $healthScore }}/100</div>
                            </div>
                            <span
                                class="ops-badge {{ $healthScore >= 80 ? 'ops-ok' : 'ops-warn' }}">{{ $healthScore >= 80 ? 'Good' : 'Attention' }}</span>
                        </div>
                        <div class="ops-status d-flex justify-content-between align-items-center">
                            <div>
                                <div class="ops-status-head">Traffic Status</div>
                                <div class="ops-status-value">{{ $orders24h > 0 ? 'Active' : 'Low' }}</div>
                            </div>
                            <span class="ops-badge {{ $orders24h > 0 ? 'ops-ok' : 'ops-neutral' }}">24h</span>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <tbody>
                                    @forelse($recentOrders as $order)
                                        <tr>
                                            ...
                                            <td class="font-monospace">{{ $order->order_number ?: $order->public_id }}</td>
