...
<header class="header">
    ...
    <nav class="navbar navbar-expand-lg header-navbar py-0">
        <div class="container-fluid header-shell">
            <a href="{{ route('shop') }}" class="navbar-brand logo-container me-0">
                <div class="logo-main">
            ...
            <div class="header-mobile-actions">
                <a href="/cart" class="header-mobile-cart {{ request()->is('cart*') ? 'active' : '' }}" aria-label="Open cart">
            ...
            <div class="collapse navbar-collapse header-collapse" id="mainHeaderNav">
                <ul class="navbar-nav ms-auto align-items-lg-center header-nav-list">
                    @foreach ($navLinks as $link)
                        <li class="nav-item">
                            <a href="{{ $link['href'] }}" class="nav-link header-link fw-normal {{ $link['active'] ? 'active' : '' }}">
                                <i class="bi {{ $link['icon'] }}"></i>
                                <span class="header-link-label">{{ $link['label'] }}</span>
                    ...
                    <li class="nav-item">
                        <a href="/cart" class="nav-link header-link fw-normal {{ request()->is('cart*') ? 'active' : '' }}">
                            <i class="bi bi-cart-fill"></i>
                    ...
                    @auth
                        <li class="nav-item">
                            <a href="{{ $dashboardRoute }}" class="nav-link header-link fw-normal {{ $isDashboardActive ? 'active' : '' }}">
                    ...
                    @guest
                        <li class="nav-item">
                            <a href="/login" class="nav-link header-link header-link-accent fw-normal {{ request()->is('login') ? 'active' : '' }}">
